"""Industry board dimension router.

Provides a unified interface for fetching industry board data.

Usage:
    from astock_analysis.dimensions.industry import fetch_industry

    response = fetch_industry()
    print(f"Boards: {len(response['data'])}, Provider: {response['provider']}")
"""

from __future__ import annotations

import pandas as pd

from astock_analysis.core.chain import try_chain, register_provider, ProviderError
from astock_analysis.core.config import get_config
from astock_analysis.core.cache import TTL_STATIC
from astock_analysis.providers.akshare import provider as akshare_provider

from .types import IndustryResponse

register_provider("akshare", akshare_provider)


def fetch_industry(board_name: str = "", use_cache: bool = True) -> IndustryResponse:
    """Fetch industry board data.

    Args:
        board_name: Optional industry board name filter
        use_cache: Whether to use the cache layer (default True)

    Returns:
        IndustryResponse with data list and metadata.
    """
    config = get_config()
    cache_ttl = TTL_STATIC if use_cache else None

    df, provider_name = try_chain(
        method_name="fetch_industry",
        dimension="industry",
        board_name=board_name,
        ticker=board_name or "market",
        cache_ttl=cache_ttl,
        config=config,
    )

    if not isinstance(df, pd.DataFrame):
        raise ProviderError(
            f"Provider '{provider_name}' returned {type(df).__name__} instead of DataFrame"
        )

    records = df.to_dict(orient="records")
    return IndustryResponse(
        data=[{k: (None if pd.isna(v) else v) for k, v in r.items()} for r in records],
        provider=provider_name,
        board_name=board_name,
    )
