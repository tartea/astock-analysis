...
from market_data.core.chain import ProviderError
...
class AkshareProvider:
    ...
    def _parse_chinese_amount(value: str) -> float:
        ...
        if raw.endswith("亿"):
            try:
                ...
                pass
        logger.warning("无法解析中文金额: %r", value)
        return 0.0
    ...
    def _fetch_kline_tencent(
        ...
        logger.info(
            "akshare K线 (腾讯): %s rows for %s (%s → %s)",
            len(df),
            code,
            start,
            end,
    ...
    def fetch_financials(
        ...
        logger.info(
            "akshare 财务数据: %s rows for %s", len(df), code
        )
        return df
    ...
    def fetch_capital_flow(self, code: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                "akshare 资金流向 失败 (%s)，回退到 urllib: %s", code, e
            )
            try:
                ...
                logger.warning(
                    "urllib 资金流向 失败 (%s)，回退到同花顺: %s", code, e2
                )
        ...
        df = df.rename(columns={k: v for k, v in col_map.items() if k in df.columns})
        logger.info("akshare 资金流向: %s rows for %s", len(df), code)
        return df
    ...
    def fetch_lhb(
        ...
        if code:
            df = df[df["代码"] == code]
            if df.empty:
                logger.info("akshare 龙虎榜: no records for %s", code)
                return pd.DataFrame()

        logger.info("akshare 龙虎榜: %s rows", len(df))
        return df
    ...
    def fetch_lhb_detail(
        ...
        logger.info(
            "akshare 龙虎榜详情: %s desks for %s on %s",
    ...
    def fetch_sentiment(self) -> dict:
        ...
        try:
            ...
            logger.warning("东方财富 市场情绪 失败，回退到新浪")
            try:
        ...
        logger.info(
            "akshare 市场情绪: rise=%s fall=%s limit_up=%s limit_down=%s",
    ...
    def fetch_index(
        ...
        try:
            ...
            logger.warning("东方财富 指数 失败，回退到腾讯: %s", code)
            try:
        ...
        for col in ("volume", "amount"):
            if col not in df.columns:
                df[col] = 0.0
        logger.info("akshare 指数: %s rows for %s", len(df), code)
        return df
    ...
    def fetch_industry(self, board_name: str = "") -> pd.DataFrame:
        ...
        try:
            if board_name:
                ...
                try:
                    ...
                    logger.warning("东方财富 行业板块 失败，回退到同花顺")
        ...
        logger.info("akshare 行业板块: %s rows", len(df))
        return df
    ...
    def fetch_concept(self, board_name: str = "") -> pd.DataFrame:
        ...
        try:
            if board_name:
                ...
                try:
                    ...
                    logger.warning("东方财富 概念板块 失败，回退到同花顺")
        ...
        logger.info("akshare 概念板块: %s rows", len(df))
        return df
    ...
    def fetch_margin(
        ...
        if (end - start).days > max_days:
            logger.warning(
                "akshare 融资融券: 日期范围 %s → %s 超过 %s 天，截断至最近 %s 天",
                ...
                for market, fetcher in [
                    ...
                    try:
                        ...
                        if mkt_df is not None and not mkt_df.empty:
                            ...
                            dfs.append(mkt_df)
                            logger.info(
                                "akshare 融资融券 [%s/%s] %s %s: %s rows",
                                day_idx, total_days, date_str, market, len(mkt_df),
                            )
                        else:
                            logger.info(
                                "akshare 融资融券 [%s/%s] %s %s: empty",
                                day_idx, total_days, date_str, market,
                            )
                    except ValueError:
                        # Non-trading day — akshare raises ValueError for SSE
                        logger.info(
                            "akshare 融资融券 [%s/%s] %s %s: non-trading day",
                            day_idx, total_days, date_str, market,
                        )
                    except Exception as e:
                        logger.warning(
                            "akshare 融资融券 [%s/%s] %s %s: %s",
                            day_idx, total_days, date_str, market, e,
                        )
        ...
        logger.info("akshare 融资融券: %s rows", len(df))
        return df
    ...
    def fetch_block_trade(
        ...
        if code:
            ...
            if df.empty:
                logger.info("akshare 大宗交易: no records for %s", code)
                return pd.DataFrame()

        logger.info("akshare 大宗交易: %s rows", len(df))
        return df
    ...
    def fetch_holder(self, code: str) -> pd.DataFrame:
        ...
        df = df.rename(columns={k: v for k, v in col_map.items() if k in df.columns})
        logger.info("akshare 股东数据: %s rows for %s", len(df), code)
        return df
    ...
    def fetch_top_holders(
        ...
        logger.info("akshare 十大股东(free=%s): %s rows for %s", free, len(df), code)
        return df
    ...
    def fetch_north_flow(self, code: str = "") -> pd.DataFrame:
        ...
        logger.info("akshare 北向资金: %s rows", len(df))
        return df
    ...
    def fetch_news(self, code: str = "") -> pd.DataFrame:
        ...
        logger.info("akshare 新闻: %s rows for %s", len(df), code)
        return df
    ...
    def fetch_fund(self, code: str = "") -> pd.DataFrame:
        ...
        logger.info("akshare 基金: %s rows", len(df))
        return df
    ...
    def fetch_ipo(self) -> pd.DataFrame:
        ...
        logger.info("akshare 新股: %s rows", len(df))
        return df
    ...
    def fetch_futures(self, code: str = "") -> pd.DataFrame:
        ...
        logger.info("akshare 期货: %s rows", len(df))
        return df
    ...
    def fetch_bond_convertible(self) -> pd.DataFrame:
        ...
        logger.info("akshare 可转债: %s rows", len(df))
        return df
